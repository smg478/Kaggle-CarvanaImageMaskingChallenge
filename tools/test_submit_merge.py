...
cand1 = pd.read_csv('submit/unet_11_960_ShiftFlipHue_hardExmpl_testSplit20.07-0.99652-0.99641.csv.gz', compression='gzip')
cand1 =cand1["rle_mask"]

cand2 = pd.read_csv('submit/unet_08_960__ShiftFlipHue_hardExmpl_testSplit20.06-0.99643-0.99631.csv.gz', compression='gzip')
...
for i in range(0, len(ids_test)):
    ...
    pred_merge=pred_merge> 1.1
    rle=run_length_encode(pred_merge)
    rles.append(rle)
    if i% 1000==0:
        print ('%d' %i) #, pred_merge, pred_merge.shape
...
df.to_csv('submit/submission_merge08_11.csv.gz', index=False, compression='gzip')
print("All done!")
